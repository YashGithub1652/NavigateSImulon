...
public class Simulation extends Application {
    ...
    public void start(Stage SimulationStage) {
        ...
        closeButton.setStyle("-fx-background-color: linear-gradient(to right, #43cea2,#185a9d); -fx-text-fill: white;");
        closeButton.setAlignment(Pos.BOTTOM_CENTER);
        closeButton.setOnMouseEntered(e -> closeButton.setStyle("-fx-background-color: #185a9d; -fx-text-fill: #43cea2;"));
        closeButton.setOnMouseExited(e -> closeButton.setStyle("-fx-background-color: linear-gradient(to right, #43cea2,#185a9d); -fx-text-fill: white;"));
        closeButton.setOnAction(e -> showReviewWindow(SimulationStage));


        HBox controls = new HBox(14, startButton,shareButton,favButton,closeButton);
   ...
   private void showReviewWindow(Stage ownerStage) {
    VBox root = (VBox) ownerStage.getScene().getRoot();
    GaussianBlur blur = new GaussianBlur(20);
    root.setEffect(blur);

    Stage reviewStage = new Stage();
    reviewStage.initOwner(ownerStage);
    reviewStage.initModality(Modality.WINDOW_MODAL);
    reviewStage.setTitle("Simulation Review");

    Label prompt = new Label("How was your simulation experience?");
    prompt.setFont(Font.font("Segoe UI", FontWeight.BOLD, 18));
    ...
    submit.setOnAction(e -> {
        // Handle review storage or logic here
        reviewStage.close();
        root.setEffect(null);
    });

    VBox dialogVBox = new VBox(16, prompt, reviewField, submit);
    dialogVBox.setAlignment(Pos.CENTER);
    dialogVBox.setPadding(new Insets(32));
    dialogVBox.setStyle("-fx-background-radius: 16; -fx-background-color: linear-gradient(to right, #8357a4ff, #18081fff);");

    Scene dialogScene = new Scene(dialogVBox, 420, 240);
    reviewStage.setScene(dialogScene);

    reviewStage.setOnHidden(ev -> root.setEffect(null));
